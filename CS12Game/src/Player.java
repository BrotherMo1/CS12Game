...
public class Player extends Entity {
	...
	public enum PlayerState {
		IDLE, RUN, JUMP, FALL, PORTAL, HURT, DEATH
	}
	...
	public void printAnimationDebug() {
		System.out.println("State: " + state + ", Index: " + animationIndex + ", Finished: " + animationFinished
				+ ", dy: " + dy + ", onGround: " + onGround);
	}
	...
	private void loadAnimations() {
		...
		portalFrames = loadFrames("sprites/animations/portal/portal", 20);
	...
	private void updateAnimationState() {
		...
		if (state == PlayerState.DEATH)
			return; // never change
		if (state == PlayerState.HURT && !animationFinished)
			return; // wait till hurt finishes
	...
	private void updateAnimationFrame() {
		...
		if (now - lastFrameTime > frameSpeed) {
			...
			if (state == PlayerState.JUMP || state == PlayerState.FALL || state == PlayerState.HURT
	...
	public void takeSpikeDamage(int value) {
		long now = System.currentTimeMillis();
		if (now - lastDamageTimeSpike < damageCooldownSpike)
			return;
